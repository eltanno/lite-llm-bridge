...
- **D-02:** Default the `claude-haiku` backend to `deepseek-v4-pro` (current newest DeepSeek on Ollama Cloud, tool-calling confirmed; user choice 2026-05-20, supersedes RESEARCH's earlier `qwen3-coder-next`). MUST verify tool-calling fidelity through LiteLLM during the agentic spike (OLL-02); if it lacks fidelity, swap per the fallback chain `qwen3.5` → `glm-5.1` (all current, tool-capable). Tool support is the deciding criterion, not model size. The exact cloud tag suffix is confirmed by the E-02 curl gate.
